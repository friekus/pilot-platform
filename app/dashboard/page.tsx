    ...
    async function load() {
      ...
      if (profile) {
        ...
        if (profile.access_expires_at) {
          const d = new Date(profile.access_expires_at);
          setExpiresAt(d.toLocaleDateString("en-AU", { day: "numeric", month: "long", year: "numeric" }));
        }
      }
      setLoading(false);
    }
      ...
      <div className="quiz-container">
        ...
        {tier === "founding_pilot" && (
          <div style={{
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
              <div style={{
                padding: "4px 12px", borderRadius: 100, background: "rgba(0,212,170,0.12)",
                fontSize: 12, fontWeight: 600, color: "#00D4AA",
              }}>Founding Pilot</div>
              <span style={{ fontSize: 13, color: "#6B7B8D" }}>Full access — free for 12 months</span>
            </div>
            {expiresAt && (
              <span style={{ fontSize: 12, color: "#4A5568" }}>Expires {expiresAt}</span>
            )}
          </div>
        )}
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 12, marginBottom: 32 }}>
          <DashCard
            ...
            title="Study hub"
            desc="Practice exams by licence level and subject. RPL, PPL, CPL, and IREX."
          />
          <DashCard
            href="/careers"
            accent="#5D9CEC"
            ...
            title="Career hub"
            desc="Interactive operator map. See who's hiring and where the jobs are."
          />
          <ShareDashCard />
        </div>
        ...
        {/* Quick stats */}
        <div style={{ marginBottom: 32 }}>
          <h3 style={{ fontSize: 15, fontWeight: 600, fontFamily: "'Space Grotesk', sans-serif", color: "#FFF", margin: "0 0 14px" }}>Platform stats</h3>
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 10 }}>
            {[
              { label: "Questions", value: "112+", sub: "across 7 subjects" },
              { label: "Licence levels", value: "4", sub: "RPL to IREX" },
              { label: "Operators", value: "21", sub: "mapped across AU" },
            ].map((s, i) => (
              <div key={i} style={{
                ...
                <div style={{ fontSize: 22, fontWeight: 700, color: "#FFF", fontFamily: "'Space Grotesk', sans-serif" }}>{s.value}</div>
                <div style={{ fontSize: 12, color: "#6B7B8D", marginTop: 4 }}>{s.label}</div>
